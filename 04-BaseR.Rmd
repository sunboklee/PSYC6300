...
* Calculate the variance of `a` by directly calculating the following expression: $[(a_1 - \bar{a})^2 + (a_2 - \bar{a})^2 + ... (a_n - \bar{a})^2]/(n-1)$, where $a = (a_1, a_2, ... , a_n)$

* Standardize the vector `a`, i.e., $z = \frac{x-\bar{x}}{sd(x)}$. 
